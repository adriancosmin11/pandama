...
export default function HomePage() {
    ...
    return (
        <div className="relative">
            ...
            <section className="relative min-h-[90vh] flex items-center overflow-hidden pt-20 pb-20">
                ...
                <div className="absolute inset-0 z-0">
                    <img
                        src={mistBg}
                        alt=""
                        className="absolute inset-0 w-full h-full object-cover opacity-100 sm:opacity-60 sm:mix-blend-screen scale-110"
                    />
                        ...
                        {loading ? (
                            [...Array(4)].map((_, i) => (
                                <div key={i} className="bg-white/[0.03] border border-white/5 rounded-2xl p-6 animate-pulse">
                                    <div className="w-full aspect-square mb-6 bg-white/[0.05] rounded-xl" />
            ...
            <AnimatePresence>
                {selectedProduct && (
                    <div className="fixed inset-0 z-[100] flex items-start justify-center px-6 pt-24 pb-12 overflow-y-auto">
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                        ...
                        <motion.div
                            ...
                            className="bg-black/40 backdrop-blur-3xl border border-white/10 max-w-5xl w-full rounded-3xl overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.8)] relative z-10 flex flex-col lg:flex-row lg:min-h-[600px]"
                        >
                            <button onClick={() => setSelectedProduct(null)} className="absolute top-6 right-6 lg:top-8 lg:right-8 text-white/20 hover:text-white transition-colors z-20">
                                <X className="w-8 h-8 stroke-[1.5]" />
                            </button>

                            <div className="lg:w-1/2 bg-gradient-to-b from-white/[0.03] to-transparent flex items-center justify-center p-8 lg:p-12 relative overflow-hidden min-h-[300px] lg:min-h-0">
                                <div className="absolute inset-0 bg-primary/10 blur-[100px] -z-10" />
                                <img className="max-h-[300px] lg:max-h-[500px] object-contain drop-shadow-[0_20px_40px_rgba(0,0,0,0.5)] scale-110" src={selectedProduct.image || "/placeholder-kendama.png"} alt={selectedProduct.name} />
                            </div>

                            <div className="lg:w-1/2 p-8 pt-10 lg:p-16 text-white flex flex-col justify-center">
                                <div className="mb-4 lg:mb-6">
                                    <span className="text-[8px] lg:text-[10px] font-black uppercase tracking-[0.4em] text-primary bg-primary/10 px-4 py-1.5 rounded-full border border-primary/20">{selectedProduct.category} COLLECTION</span>
                                </div>
                                <h2 className="text-4xl lg:text-6xl font-black mb-4 lg:mb-6 italic tracking-tighter uppercase leading-none">{selectedProduct.name}</h2>
                                <div className="flex items-center gap-4 lg:gap-6 mb-6 lg:mb-8">
                                    <span className="text-4xl lg:text-5xl font-black text-white/90 tracking-tighter">${selectedProduct.price.toFixed(2)}</span>
                                    {selectedProduct.originalPrice && (
                                        <span className="text-white/20 line-through text-lg lg:text-xl font-bold italic tracking-tight">${selectedProduct.originalPrice.toFixed(2)}</span>
                                    )}
                                </div>
                                <p className="text-white/40 text-xs lg:text-sm mb-8 lg:mb-12 leading-relaxed font-medium uppercase tracking-wide max-w-md">{selectedProduct.description}</p>

                                <div className="space-y-4 lg:space-y-6 mb-8 lg:mb-12">
                                    <h4 className="text-[8px] lg:text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Specificații Tehnice</h4>
                                    <div className="grid grid-cols-2 gap-x-6 lg:gap-x-8 gap-y-3 lg:gap-y-4">
                                        {selectedProduct.specs.map(spec => (
                                            <div key={spec} className="flex items-center gap-2 lg:gap-3 text-[8px] lg:text-[10px] font-black uppercase tracking-widest text-white/60">
                                ...
                                <div className="flex gap-4 lg:gap-6">
                                    <button
                                        ...
                                        className="btn-primary flex-1 py-4 lg:py-6 rounded-xl text-[10px] lg:text-xs font-black uppercase tracking-widest hover:scale-[1.02] transition-all shadow-[0_20px_40px_rgba(76,175,80,0.2)] active:scale-95 flex items-center justify-center gap-3 lg:gap-4"
                                    >
                                        <ShoppingCart className="w-4 h-4 lg:w-5 lg:h-5 stroke-[2]" /> ADAUGĂ ÎN COȘ
                                    </button>
                                    <button className="w-14 h-14 lg:w-20 lg:h-20 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center hover:bg-white/10 transition-all group shrink-0">
                                        <Heart className="w-5 h-5 lg:w-6 lg:h-6 text-white/40 group-hover:text-red-500 transition-colors" />
                                    </button>
                                </div>
                            </div>
                        </motion.div>
